...
namespace {
	...
	Mat lucasKanade(Mat imgA, Mat imgB, int xsarea, int ysarea, int xarea, int yarea, deque<Point> corners, bool verbose = true)
	{
		...
		//cout << string("\nWriting lkoutput to LKOUT");
		//imwrite(string("o") + string("LKOUT"), outimg);
	...
	int process(cv::VideoCapture& capture) 
	{
		...
		for (;;) 
		{
			...
			deque<Point> corn = findCorners(prevGray, xarea, yarea, thres, false);

			image=lucasKanade(prevGray, gray, 3, 3, xarea, yarea, corn, true);
...
{  
	...
	//		lucasKanade(previmg, curimg, 3, 3, xarea, yarea, corn, true);
	//	}
	//}
